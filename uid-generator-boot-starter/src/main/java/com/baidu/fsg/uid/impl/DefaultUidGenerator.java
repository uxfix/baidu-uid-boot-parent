...
public class DefaultUidGenerator implements UidGenerator, InitializingBean {
    ...
    public void setWorkerIdAssigner(WorkerIdAssigner workerIdAssigner) {
        Assert.notNull(workerIdAssigner, "WorkerIdAssigner must be positive!");
        this.workerIdAssigner = workerIdAssigner;
    }

    public void setTimeBits(Integer timeBits) {
        if (Objects.nonNull(timeBits) && timeBits > 0) {
            this.timeBits = timeBits;
        }
    }

    public void setWorkerBits(Integer workerBits) {
        if (Objects.nonNull(workerBits) && workerBits > 0) {
            this.workerBits = workerBits;
        }
    }

    public void setSeqBits(Integer seqBits) {
        if (Objects.nonNull(seqBits) && seqBits > 0) {
